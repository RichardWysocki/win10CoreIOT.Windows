...
namespace UWP.WebViewer
{
    ...
    public sealed partial class TemplatePage : Page
    {
        ...
        private async void Page_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            var webSiteURL = "http://www.RichardWysocki.com";
            if (loadExistingData != null)
            {
                webSiteURL = loadExistingData.WebsiteURL;
            }

            try
            {
                Wv1.Navigate(new Uri(webSiteURL));
            }
            catch (Exception exception)
            {
                Console.WriteLine(exception);
                Wv1.Navigate(new Uri("http://www.RichardWysocki.com"));
            }
